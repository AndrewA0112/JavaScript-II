...
// Problem 1  // Find all runners apart of a certain company
function findCompanyRunners(company) {
    let companyRunners = [runners.filter(runner => runner.company_name === company)];
    return companyRunners;
}
console.log(JSON.stringify(findCompanyRunners('Gabtype')));

// Problem 2 // Convert each last name to uppercase because the director is extra drunk
let allCapsLast = [runners.map(runner => runner.last_name.toUpperCase())];
console.log(JSON.stringify(allCapsLast)); 

// Problem 3 // Get all the runners with the certain shirt size

function totalShirts(size) {
    let totalShirt = [runners.filter(runner => runner.shirt_size === size)];
    return totalShirt;

}
// let totalShirtSize = [runners.map()]
console.log(JSON.stringify(totalShirts('M')))